...
var webimConfigs = (function () {
    ...
    * */
    ...
    function onTeamMembers(obj) {
        ...
        // //提取有用信息，account
        // var arr = data.teamMembers[teamId], newArr = [];
        // for (var i = 0; i < arr.length; i++) {
        //     newArr[i] = arr[i].account;
        // }
        // //获取用户资料
        // getUsersInfo(newArr, function (users) {
        //     data.teamMembers[teamId] = users;
        // })
    }
    ...
    function onUpdateSession(session) {
        ...
        * */
        ...
        if (session.lastMsg.from != '4' && session.lastMsg.from != nim.account) {
            contactProjectAndSession();
        }
        //网站客服新消息通知，触发事件
        ...
        // 如果是新的会话
        if (session.to != '4' && session.to != '3') {
            // 新人的消息
            if (!hasNotInFriendList(session.to)) {
                addNewFriend(session);
            }
        }
    }
    ...
    function parseMsg(msg) {
        ...
        if (user) {
            ...
            msg.company = user.custom && JSON.parse(user.custom).com || '';
        }
        return msg
    }
    ...
    function onSyncDone() {
        ...
        var j_list = globalData.friend_list;
    ...
    function hasNotInFriendList(uid) {
        var friends = globalData.friend_list;
        var hasJoined = false;
    ...
    function addNewFriend(session) {
        var newFriend = {
            id: session.to,
            nick: session.lastMsg.fromNick,
            phone: '',
            sex: '1',
            im_unread: session.unread,
            lastMsg: session.lastMsg
        }
        // 如果是自己发的消息
        if(session.lastMsg.from == globalData.user.user_id){
            var list = globalData.project_list;
            list.forEach(function(element) {
                if(element.uid == session.to){
                    newFriend.nick = element.user.nick;
                }
            }, this);
        }
        globalData.friend_list.push(newFriend);
        contactProjectAndSession();
    }
    ...
    // 从参与的工程商列表中获取用户信息
    function getSupplierInfoByProlist(accid) {
        var slist = globalData.friend_list;
    ...
    function contactProjectAndSession() {
        var list = globalData.friend_list;
        var sessions = webimConfigs.data.sessions;
        var amt_friends_unread = 0;
        for (var i = 0; i < list.length; i++) {
            // 整合聊天信息
            for (var j = 0; j < sessions.length; j++) {
                // 好友未读
                if (list[i].id == sessions[j].to) {
                    list[i].im_unread = sessions[j].unread;
                    amt_friends_unread += sessions[j].unread;
                    if (list[i].lastMsg == '') {
                        list[i].lastMsg = sessions[j].lastMsg;
                    } else {
                        if (list[i].lastMsg.time <= sessions[j].lastMsg.time) {
                            list[i].lastMsg = sessions[j].lastMsg;
                        }
                    }
                    // 如果未读数大于9，则显示…
        ...
        globalData.unread_num.private_unread = amt_friends_unread;
    }
    return {
...
window.onbeforeunload = function () {
    ...
    // 好友列表缓存
    var friend_list = globalData.friend_list;
...
function readStorgeFriends() {
    ...
    var fri_list = globalData.friend_list;
